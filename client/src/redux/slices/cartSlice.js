...
const cartSlice = createSlice({
  ...
  reducers: {
   ...
   removeFromCart:(state,action)=>{
    ...
    const existingItemIndex=state.cartItems.findIndex((item)=>item._id === itemToRemove._id);
    ...
    state.totalPrice = Number((state.totalPrice - itemToRemove.price * itemToRemove.quantity).toFixed(2));
